...
def agent_play():
    ...
    if MODEL == 'qwen':
        ...
        print('调用零一万物大模型')
        # agent_plan_list, ai_response = agent_plan_yi(speech_result) # 单轮对话
        agent_plan_list, ai_response = agent_plan_yi_history(speech_result) # 多轮对话
    elif MODEL == 'ernie':
        print('调用文心一言大模型')
    ...
    if (start=='1') or (start=='3'):
        # 依次执行动作
        terminal = 'ssh root@{} "export DISPLAY=:0 && python ~/TonyPi/OpenVINO/utils_robot.py"'.format(PI_IP)   
        os.system(terminal)
        print('所有动作完成')
